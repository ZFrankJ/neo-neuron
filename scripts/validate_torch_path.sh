...
then
  ...
  printf 'Use requirements-lock/constraints-macos-mlx.txt for the portable PR #15 parity lock.\n'
  printf 'Use requirements-lock/constraints-mac-mini-training.txt only for the documented main-machine exception.\n'
fi
...
printf 'MPS is validated only if the MPS probe runs without skips or failures.\n'
printf 'MLX parity is validated only if the MLX preflight and MLX parity test block pass.\n'
