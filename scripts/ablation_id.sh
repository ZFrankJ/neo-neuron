...
DEVICE="${1:-auto}"
BACKEND="${2:-mlx}"
ACTIVATIONS_CSV="${3:-id3,id4,id5,tanh,gain_tanh,gelu,none}"
...
if [[ "${#ACTIVATIONS[@]}" -eq 0 ]]; then
  echo "No activations provided. Pass a CSV list like: id3,id4,id5,tanh,gain_tanh,gelu,none" >&2
  exit 1
fi
...
allowed = {"id3", "id4", "id5", "tanh", "gain_tanh", "one_plus_tanh", "1+tanh", "gelu", "none", "identity"}
if activation_name not in allowed:
    raise ValueError(
        f"Unsupported activation '{activation_name}'. Expected one of: "
        "id3,id4,id5,tanh,gain_tanh,gelu,none."
    )
...
for activation_name in "${ACTIVATIONS[@]}"; do
  ...
  case "$activation_name" in
    id3|id4|id5|tanh|gain_tanh|one_plus_tanh|1+tanh|gelu|none|identity)
      run_with_activation "$activation_name"
      ;;
    *)
      echo "Skipping unknown activation '$activation_name' (allowed: id3,id4,id5,tanh,gain_tanh,gelu,none)" >&2
      ;;
  esac
done
