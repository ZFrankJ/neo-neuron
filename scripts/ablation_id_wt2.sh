...
BASE_CFG="${ROOT_DIR}/configs/wt2/neo_6m.yaml"
...
run_with_activation() {
  ...
  local tmp_cfg
  tmp_cfg="$(mktemp -t neo_ablation_id_wt2.XXXXXX.yaml)"
...
PY
  ...
  echo "== Training: configs/wt2/neo_6m.yaml | activation=${activation_name} | full epochs ==" >&2
  if [[ -n "$BACKEND" ]]; then
